...
*文献検索*から*ナビゲーション可能なナレッジグラフ構築*までを全工程カバーする、意見的（opinionated）なエンドツーエンドツールキットです。すべて Claude Code 内で完結します。現在の beta は `v0.3.0-beta.2` です。

この公開リポジトリは、ローカルのプライベート Claude 環境のミラーではありません。ローカル専用の skills、agents、認証情報、実行 ID、ログ、private URL、マシン固有パスは公開リリースに含めません。
...
./scripts/install.sh --profile researcher    # 検索 + 処理 + 分析 + 執筆
./scripts/install.sh --profile writer        # 執筆 + プレゼンテーション
./scripts/install.sh --profile knowledge     # ナレッジ管理 + プレゼン
./scripts/install.sh --profile full          # すべて、Phase 01 から Phase 07 まで
...
| `researcher` | 01, 02, 03, 04 | 25 | 14 | 文献検索、分析、ドラフト作成 |
| `writer` | 04, 06 | 16 | 7 | 学術執筆、改訂、スライド、図表 |
| `knowledge` | 05, 06 | 17 | 2 | ナレッジ管理と Obsidian |
| `full` | 01-07 | 43 | 16 | active beta の `07-pipeline` を含む完全ツールキット |

`./scripts/install.sh --profile <name>` でインストールするか、`--module <phase>` で個別モジュールをインストールしてください。

公開 catalog は [docs/tool-catalog.md](docs/tool-catalog.md) と [docs/tool-catalog.zh-CN.md](docs/tool-catalog.zh-CN.md) に分離されています。catalog はこのリポジトリ内の公開 modules、skills、agents、公開 integration のみを扱い、ローカル専用の Claude 環境や private runtime の一覧ではありません。
...
- **v0.3.0-beta.2 public refresh** -- README、profile installer、CHANGELOG、tool catalog を更新し、ローカル専用 skills/agents は移行しません。
- **Release metadata guardrails** -- alpha、beta、rc tag は GitHub prerelease となり、Latest にはなりません。
- **Release body safety scan** -- release notes は公開前に安全スキャンされます。
- **Five install profiles** -- `minimal`、`researcher`、`writer`、`knowledge`、`full` を JSON、Bash、PowerShell、verifier で整合させています。
...
- **拡張されたスキルセット** -- 全フェーズに研究、執筆、ナレッジ管理、プレゼンテーション機能を追加
- **16 エージェント** -- 文献レビューアー、LaTeX スペシャリスト、リバタルライターなど
- **インストールプリセット** -- 必要なモジュール構成だけをインストール
