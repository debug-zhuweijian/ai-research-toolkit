...
2. 选择你的 Obsidian Vault 目录（如 `D:\obsidian\`）
3. 确认打开

**知识库 Vault**（Graphify 驱动）：

1. 点击 "Open folder as vault"
2. 选择你的知识库目录（如 `D:\knowledge\`）
3. 确认打开
...
Knowledge Base 相关脚本位于本仓库的 `scripts/` 目录，详见 `knowledge-base-README.md`。
...
在不同位置之间同步知识库内容：
- `<CLAUDE_DOCS>/` → `<KNOWLEDGE_BASE>/raw/` 的资料同步
- Git 仓库提交和推送
...
```
/graphify <YOUR_KNOWLEDGE_BASE_PATH>
```
...
wing: graph-learning
room: gnn-methods
content: "Smith2024 提出了一种基于 Transformer 的图神经网络方法，
在 15 个图分类任务上达到 92% 的准确率..."
```

#### 语义搜索

```
请用 mempalace_search 搜索 "图神经网络最新方法"
```
...
```
请用 mempalace_kg_timeline 查看 "图神经网络" 的时间线
```

#### 漫游宫殿

```
请用 mempalace_traverse 从 "gnn-methods" 房间开始漫游，查看跨领域连接
```
...
└── base/                   ← 纯笔记和文档内容
    ├── Smith2024_Graph_Neural_Networks/
    │   ├── Smith2024_Graph_Neural_Networks_EN.pdf
    │   ├── Smith2024_Graph_Neural_Networks_EN.md
    │   ├── Smith2024_Graph_Neural_Networks_review.md    ← paper-review 产出
    │   └── Smith2024_Graph_Neural_Networks_notes.md     ← 个人笔记
    ├── DeepLearning_GraphLearning/
    │   └── research_report.md                     ← deep-research 产出
    └── 日常笔记.md
```

### 4.2 知识库 Vault（<KNOWLEDGE_BASE_PATH>）
...
| 素材来源 | 接收方式 | 存储位置 |
|----------|----------|----------|
| 论文 PDF | MinerU 转换 → 手动或自动归档 | `<OBSIDIAN_VAULT>/base/<论文名>/` |
| 审阅报告 | `/paper-review` 产出 | `<OBSIDIAN_VAULT>/base/<论文名>/*_review.md` |
| 调研报告 | `/deep-research-v5` 产出 | `<OBSIDIAN_VAULT>/base/<主题名>/` |
| 写作草稿 | `/academic-writing` 产出 | `<OBSIDIAN_VAULT>/base/<项目名>/draft/` |
| 组会 PPT | `/group-meeting-slides` 产出 | `<CLAUDE_DOCS>/ppt/` |
...
- 主窗口：`<OBSIDIAN_VAULT>`（日常工作）
- 辅助窗口：`<KNOWLEDGE_BASE_PATH>`（查阅知识图谱）
...
4. **Graphify 路径**：在 Windows 上使用 Graphify CLI 时，路径需要使用正斜杠或双反斜杠：
   ```bash
   # 正确（替换为你的知识库路径）
   graphify build D:/knowledge
   # 也正确
   graphify build "D:\\knowledge"
   ```

5. **Obsidian Vault 位置**：避免将 Vault 放在 OneDrive 或其他同步目录中，同步冲突会损坏 `.obsidian/` 配置。推荐使用本地磁盘上的独立目录。
