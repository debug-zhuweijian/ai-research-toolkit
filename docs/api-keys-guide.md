...
|---------|------|----------|----------|--------|
| Anthropic API Key | Claude Code 核心 | console.anthropic.com | $5 最低充值 | 必需 |
...
Anthropic API Key 是 Claude Code 的核心认证凭据。没有这个 Key，Claude Code 无法调用任何 AI 模型。所有 Skill 和 MCP 工具的 AI 推理能力都依赖此 Key。
...
**解决**：
1. 检查 Key 是否完整复制（以 `sk-ant-api03-` 开头）
2. 确认没有多余的空格或换行符
3. 在 [console.anthropic.com](https://console.anthropic.com) 检查 Key 状态
...
4. **检查环境变量**：确认 `ANTHROPIC_API_KEY` 等环境变量正确设置
