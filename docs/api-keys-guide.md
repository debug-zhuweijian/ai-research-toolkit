...
如果你使用 Anthropic 兼容端点，需要同时设置 API Key 和 `base_url`。公开文档只使用占位符，私有端点、私有模型名和本机实际值请留在本机配置中：
...
2. **使用环境变量**而非硬编码。推荐在 `~/.bashrc` 中设置 Key，然后在 `~/.claude.json` 中引用：
   ```bash
   # ~/.bashrc
   export ANTHROPIC_API_KEY="<ANTHROPIC_API_KEY>"  # 或兼容端点的 Key
   # 如果使用兼容端点（如智谱 BigModel），还需要设置：
   # export ANTHROPIC_BASE_URL="<ANTHROPIC_COMPATIBLE_BASE_URL>"
   export BIGMODEL_API_KEY="<BIGMODEL_API_KEY>"
