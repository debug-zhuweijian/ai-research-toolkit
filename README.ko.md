...
본 툴킷은 *논문 검색*부터 *탐색 가능한 지식 그래프 구축*까지 전 과정을 Claude Code 내에서 수행할 수 있도록 설계된 의견이 반영된 엔드투엔드 툴킷입니다. 현재 beta는 `v0.3.0-beta.2`입니다.

이 공개 저장소는 로컬 비공개 Claude 환경의 미러가 아닙니다. 로컬 전용 skills, agents, credentials, runtime IDs, logs, private URLs, machine-specific paths는 공개 릴리스에 포함하지 않습니다.
...
./scripts/install.sh --profile researcher    # 검색 + 처리 + 분석 + 작성
./scripts/install.sh --profile writer        # 작성 + 프레젠테이션
./scripts/install.sh --profile knowledge     # 지식 관리 + 프레젠테이션
./scripts/install.sh --profile full          # 전체 설치, Phase 01부터 Phase 07까지
...
| `researcher` | 01, 02, 03, 04 | 25 | 14 | 문헌 검색, 분석, 초안 작성 |
| `writer` | 04, 06 | 16 | 7 | 학술 작성, 수정, 슬라이드, 도표 |
| `knowledge` | 05, 06 | 17 | 2 | 지식 관리 및 Obsidian |
| `full` | 01-07 | 43 | 16 | active beta `07-pipeline`을 포함한 전체 툴킷 |

`./scripts/install.sh --profile <이름>`으로 설치하거나 `--module <단계>`로 개별 모듈을 설치하십시오.

공개 catalog는 [docs/tool-catalog.md](docs/tool-catalog.md) 및 [docs/tool-catalog.zh-CN.md](docs/tool-catalog.zh-CN.md)에 분리되어 있습니다. catalog는 이 저장소에 포함된 공개 modules, skills, agents, public integrations만 다루며 로컬 전용 Claude 환경이나 private runtime 목록이 아닙니다.
...
- **v0.3.0-beta.2 public refresh** -- README, profile installers, CHANGELOG, tool catalog를 업데이트하며 로컬 전용 skills/agents는 이전하지 않습니다.
- **Release metadata guardrails** -- alpha, beta, rc tag는 GitHub prerelease로 게시되고 Latest가 되지 않습니다.
- **Release body safety scan** -- release notes는 게시 전에 안전 스캔을 통과해야 합니다.
- **Five install profiles** -- `minimal`, `researcher`, `writer`, `knowledge`, `full`을 JSON, Bash, PowerShell, verifier에서 일관되게 유지합니다.
...
- **확장된 스킬 세트** -- 모든 단계에 연구, 작성, 지식 관리, 프레젠테이션 기능 추가
- **16개 에이전트** -- 문헌 리뷰어, LaTeX 전문가, 리벗털 작성자 등
- **설치 프리셋** -- 필요한 모듈 조합만 설치
